...
public class Administrator extends User implements login_func {
    ...
    public void resolved_complaint() {
        ...
        for (Complaint c : this.complaints) {
            if (c.getComplaint_id() == id) {
                ...
                JOptionPane.showMessageDialog(null, "Complaint_ID: " + c.getComplaint_id() + " resolved successfully.", "Resolved Complaint", JOptionPane.INFORMATION_MESSAGE);
                break;
            }
        }
    }
    ...
    public void manage_course_catalog(Administrator admin) {
        boolean exit = false;
        while (!exit) {
            String[] options = {"Add course", "View all courses", "Delete course", "Exit"};
